...
export default function ResetPassword() {
  ...
  const submit = async (e) => {
    ...
    try {
      ...
      setMessage('Password reset. You may sign in now.');
      setTimeout(() => navigate('/login'), 1200);
    } catch (err) {
      setMessage(err.response?.data?.message || 'Unable to reset');
    }
    setLoading(false);
  };
  ...
  return (
    <AuthShell
      ...
      title="Set a new secure password and return to your workspace."
      subtitle={`Resetting access for ${email || 'your account'} with a polished recovery flow.`}
